    ...
    <div id="settings-background">
      <div id="settings-window">
        <div class="nav-column">
          <button on:click={() => setActiveTab("Login/Register/Sync")}
            >Login/Register/Sync</button
          >
          <button on:click={() => setActiveTab("workspace data")}
            >Workspace Data</button
          >
          <button on:click={() => setActiveTab("windows data")}
            >Windows Data</button
          >
          <button on:click={() => setActiveTab("theme settings")}
...
<style>
  ...
  button {
    ...
    width: calc(
      100% - 10px
    ); /* Adjust button width considering padding and margin */
  }
</style>

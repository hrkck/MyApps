<script>
  ...
  import { getLocalSvelteStore } from "../../store";

  export let windowId;
  // export let windowIconSrc = "";
  export let windowIconUrl = "";
  // export let url = "";
  export let showIcon = true;

  const LinkAppStore = getLocalSvelteStore("linkAppStore-" + windowId, {
    url: windowIconUrl,
  });
...
{#if showIcon}
  <a
    ...
    on:click={iconClick}
  >
    <img src={windowIconUrl + "/favicon.ico"} alt="Icon" class="icon" />
  </a>
{:else}
  ...
  <input
    type="text"
    id="url"
    bind:value={$LinkAppStore.url}
    on:input={updateImageUrl}
  />

  {#if windowIconUrl !== ""}
    <img
      src={windowIconUrl + "/favicon.ico"}
      alt="Favicon"
      class="favicon-image"
    />
  {:else}
